...
void StartControlTask(void const * arguments)
{
  ...
  const portTickType HYPERPERIOD;  
...
/* Delay the task until a fixed time to ensure constant execution frequncy */
    vTaskDelayUntil(&last_task_start,HYPERPERIOD/2); //(EXAMPLE: "HYPERPERIOD/2" runs 2 timer per hyperperiod)
  }
}
